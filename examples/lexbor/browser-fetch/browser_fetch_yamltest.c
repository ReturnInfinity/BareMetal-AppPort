// browser_fetch_yamltest.c -- minimal-DOM isolation repro for the
// parked GC-assert crash (see BROWSER.md's "identify the leaked
// objects"/yamltest.c sections). yamltest.c/yamltest2.c/yamltest3.c
// already ruled out plain QuickJS + real js-yaml source (single eval,
// sequential multi-eval, curl-buffer-loaded) as sufficient on their
// own -- 63 boots, zero leaks. This file is the next concrete step
// those left identified: run the SAME real js-yaml source through the
// ACTUAL production `run_scripts()`/`run_one_script()`/`eval_script()`
// code path from browser_fetch.c (byte-for-byte identical -- copied,
// not reimplemented, so the DOM<->QuickJS binding layer is exercised
// exactly as it is in production), but against a tiny embedded HTML
// document instead of a live fetch of Swagger UI's real multi-KB
// bundle from https://httpbin.org/.
//
// Only main() differs from browser_fetch.c: no curl fetch of a page,
// no argv[1] URL. Builds `<html><body><script>` + the real js-yaml
// bytes (embedded verbatim from js_yaml_src.h, not re-typed) +
// `</script></body></html>` as one in-memory, explicitly-length-
// tracked buffer (never a null-terminated C string -- same convention
// as every fetch-based example, even though this content isn't
// fetched), and parses that with lexbor exactly the way a real fetched
// page's bytes are parsed. No <script src>, so resolve_script_url()/
// run_external_script()'s network-fetch path is present (copied
// verbatim, for fidelity) but never exercised here -- this test is
// specifically about DOM presence + one real inline script, not
// external-script fetching (already covered by browser_fetch.c
// itself).
//
// Point: does a MINIMAL DOM (one inline <script> tag, real js-yaml
// content) reproduce the `Assertion failed: list_empty(&rt->gc_obj_list)`
// leak that showed up against Swagger UI's real bundle? If yes, DOM
// presence + the Element/document/window bindings (not bundle size,
// content, or the live-fetch path) is the real trigger. If no, the
// leak still needs something specific to the real-world case that
// this minimal repro doesn't capture.
...
// querySelectorAll/getElementById -- see browser.c's matching comments
// for the full rationale (real JS Array via JS_NewArray()+
// JS_SetPropertyUint32(), verified .length/indexing; getElementById as
// a direct tree walk rather than a "#"+id selector, to match exact
// attribute-string-equality semantics without CSS-selector escaping
// edge cases).
struct qsa_result {
	JSContext *ctx;
	JSValue arr;
...
// Only real difference from browser_fetch.c: no live page fetch. Builds
// `<html><body><script>` + the real js-yaml bytes + `</script></body>
// </html>` as one in-memory buffer with an explicit length (never a
// null-terminated C string, same convention every fetch-based example
// uses even though nothing here is actually fetched) -- confirmed by a
// one-off host-side check that js-yaml's real minified source contains
// neither "</script" nor "<!--" anywhere, so it can't prematurely close
// the tag it's embedded in.
static const char DOC_PREFIX[] = "<html><body><script>";
static const char DOC_SUFFIX[] = "</script></body></html>";

static char *build_minimal_doc(size_t *out_len)
{
	size_t total = sizeof(DOC_PREFIX) - 1 + js_yaml_min_js_len + sizeof(DOC_SUFFIX) - 1;
	char *buf = malloc(total);
	if (!buf)
		return NULL;

	size_t off = 0;
	memcpy(buf + off, DOC_PREFIX, sizeof(DOC_PREFIX) - 1);
	off += sizeof(DOC_PREFIX) - 1;
	memcpy(buf + off, js_yaml_min_js, js_yaml_min_js_len);
	off += js_yaml_min_js_len;
	memcpy(buf + off, DOC_SUFFIX, sizeof(DOC_SUFFIX) - 1);
	off += sizeof(DOC_SUFFIX) - 1;
...
int main(void)
{
	printf("BareMetal browser -- lexbor + QuickJS (minimal-DOM js-yaml isolation repro)\n");
	printf("No live fetch -- one embedded <script> with real js-yaml 4.1.0 source (%u bytes)\n\n",
	       js_yaml_min_js_len);

	curl_global_init(CURL_GLOBAL_DEFAULT);

	size_t resp_len = 0;
	char *resp_data = build_minimal_doc(&resp_len);
	if (!resp_data) {
		printf("build_minimal_doc() failed (out of memory)\n");
		curl_global_cleanup();
		return 1;
	}
	...
	// No <script src> in this document, so resolve_script_url()/
	// run_external_script() (copied verbatim from browser_fetch.c for
	// fidelity) are never exercised -- g_base_url is still set up the
	// same way for teardown-path consistency, just against a placeholder
	// rather than a real post-redirect URL (there was no redirect, there
	// was no fetch).
	lxb_url_parser_init(&g_url_parser, NULL);
	static const char PLACEHOLDER_URL[] = "http://localhost/yamltest";
