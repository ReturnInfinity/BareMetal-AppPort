...
//   - MBEDTLS_SSL_PROTO_TLS1_3 / MBEDTLS_PSA_CRYPTO_C / MBEDTLS_LMS_C: off
//     (TLS 1.2 legacy-crypto-API path only, to keep the build small)
//   - MBEDTLS_TIMING_C: off (needs gettimeofday/setitimer/alarm, none of
 ...
 *
 * BareMetal-AppPort: enabled -- mbedtls_hardware_poll() is implemented in
 * port/mbedtls_port/entropy_hardware_poll.c using RDRAND (same primitive
 * crt0.c's fill_random() already uses to seed musl's stack-protector/
 * mallocng entropy), since there's no /dev/urandom or getrandom() here.
 */
 ...
 *
 * BareMetal-AppPort: enabled -- no /dev/urandom, no getrandom() (see
 * OPENISSUES.md), so the built-in poller would just fail at runtime.
 * MBEDTLS_ENTROPY_HARDWARE_ALT above supplies mbedtls_hardware_poll()
 * instead.
 */
 ...
 *
 * BareMetal-AppPort: disabled -- TLS 1.3 requires MBEDTLS_PSA_CRYPTO_C
 * (see above), which is disabled below to keep the build to the legacy
 * crypto API. TLS 1.2 (kept enabled below) is sufficient to reach the
 * vast majority of HTTPS servers.
 */
 ...
 *
 * BareMetal-AppPort: disabled -- requires MBEDTLS_PSA_CRYPTO_C (see
 * above), unused by TLS 1.2/X.509 anyway (post-quantum signature scheme,
 * not used by ordinary HTTPS servers).
 */
 ...
 *
 * BareMetal-AppPort: disabled -- its POSIX implementation needs
 * gettimeofday()/setitimer()/alarm(), none of which this port has (see
 * OPENISSUES.md). Only used for DTLS retransmission timers and
 * benchmarking, neither of which tls_shim.c (TCP-only, TLS 1.2 client)
 * needs.
 */
