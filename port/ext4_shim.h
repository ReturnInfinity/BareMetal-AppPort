...
// Sets an open file's size (SQLite's VFS layer -- see
// port/sqlite_port/ -- uses this to finalize/shrink journal files).
// Backed directly by lwext4's own ext4_ftruncate(), unlike the old
// BMFS shim's hand-rolled version -- growth zero-fills like a real
// ftruncate(), not just shrink.
long ext4_shim_truncate(long fd, size_t length);
